...
contract NFTPurchaseManager is ReentrancyGuard {
    ...
    //mapping to store the purch request
    mapping(address => uint256[]) s_fanToPurchaseReqId;
    ...
    //all the request are stored here
    //its a mapping
    // creator_id -> array of request_id
    ...
    ////////////
    //2 -> address , desc and
    //getreqDEtails-> give all the details about the reqyest takes the
    ...
    //make it a context can be read CANNOT FORGET IT!!
    event NewCreator(
        uint256 creatorId,
    ...
    function getCreator() external view returns (uint256) {
        return s_creatorId;
    }
    ...
    function purchaseNFT(
        ...
        s_fanToPurchaseReqId[msg.sender].push(s_requestCount);
        s_requestCount++;
    ...
    //pop up of the details DO NOT FORGET CINFORM BUTTON
    //MINT FUNCTION WORKING!!!! SUPER IMPORTANT

    // -------------------------------------------------

    //FAN
    //JUST GIVE CREATOR ID AND IT WIL RETURN THE DEATILS OF THE CREATOR USING THE CREATOR ID
    //PURCHASENFT PROVIDE CREATOR_ID (ONCE IM SURE IK THAT THIS IS THE CREATOR I WANT) I CAN
    // GIVE THE CREATOR_ID AND DESC/MESSAGE FOR THE NFT
    //REFUND WHICH WILL ACCEPT REQUEST_ID (BUT HOW DO YOU YOUR REQUEST ID? here they need to rem but in future scope of it they will have a nice dashboard where they can just scroll thru all the requests) IT WILL REFUND THE AMOUNT CUTTING GAS FEE OFC
    ...
    function getFanLatestReq() external view returns (uint256) {
        uint256 len = s_fanToPurchaseReqId[msg.sender].length - 1;
        return s_fanToPurchaseReqId[msg.sender][len];
    }
